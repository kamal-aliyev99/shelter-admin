...
const FindUS = () => {
  ...
  function getDatas() {
    fetch(`${apiURL}/api/findUs?lang=${lang}`)
      .then(res => {
        if (res.ok) {
          return res.json();
        } else {
    ...
    fetch(`${apiURL}/api/findUs/${id}`, {
      ...
      .then((res) => {        
        if (res.ok) {
          ...
          showNotf(false, `${res.status}: An error occurred while deleting Find Us`);
          return res.json().then(err =>{
            console.error(err);
          })
        }
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            <p className="text-body-secondary small">
              You can add, update and delete any <i>Find Us</i>
            </p>
            ...
            <CModal scrollable visible={modalVisible} onClose={() => closeModal()} className='infoModal'>
              <CModalHeader>
                <CModalTitle>Find Us</CModalTitle>
              </CModalHeader>
              <CModalBody>
            ...
            >
              ...
              <CModalBody>
                Do you want to delete <strong>{selectedData?.key}</strong> ? 
              </CModalBody>
              <CModalFooter>
